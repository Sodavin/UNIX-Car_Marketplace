...
import { FaArrowLeft, FaPrint, FaCopy } from 'react-icons/fa';
import './Productpage/Receipt.css';
...
export default function Receipt({ orderData, onBackToStore }) {
  ...
  const data = {
    ...
    items: (Array.isArray(normalizedOrder.items) && normalizedOrder.items.length > 0
      ...
      : Array.isArray(normalizedOrder.cartItems) && normalizedOrder.cartItems.length > 0
    ...
    contact: {
      type: normalizedOrder.contact?.type || normalizedOrder.contactType || 'Phone',
      value: normalizedOrder.contactValue || normalizedOrder.phone || normalizedOrder.contact?.value || normalizedOrder.customer?.phone || '+855 12 345 678',
    },
    address: {
      recipientName: normalizedOrder.customer?.fullname || normalizedOrder.full_name || normalizedOrder.address?.recipientName || 'Customer Name',
      phoneNumber: normalizedOrder.customer?.phone || normalizedOrder.phone || normalizedOrder.contact?.value || '+855 12 345 678',
      ...
      addressDetails: normalizedOrder.customer?.address || normalizedOrder.address?.addressDetails || normalizedOrder.address || '',
    },
    pricing: {
      ...
      <div className="receipt-card receipt-card--invoice">
        ...
        <section className="receipt-body receipt-body--invoice">
          ...
          <div className="invoice-details">
            ...
            <div className="detail-row">
              <span>Descript Information</span>
